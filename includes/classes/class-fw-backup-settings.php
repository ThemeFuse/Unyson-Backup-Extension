...
class FW_Backup_Settings {
	...
	public function _admin_action_admin_enqueue_scripts( $hook ) {
		...
		if ( $hook == 'edit.php' && $post_type == $this->backup()->post_type()->get_post_type() ) {
			...
			wp_localize_script( 'backup-settings', 'backup_settings_i10n', array(
					'title'   => __( 'Backup Schedule', 'fw' ),
					'options' => $options,
	...
	public function get_cron_storage_options( $cron_id, $storage_id ) {
		...
		return ( isset( $values["$cron_id-storage"] ) && isset( $values["$cron_id-storage"][ $storage_id ] ) )
			? (array) $values["$cron_id-storage"][ $storage_id ]
			: array();
	}
...
//	    return $options;
	...
	private function options_tab( $cron_id, $cron_title, $schedule_default, $schedule_choices ) {
		if ( $this->backup()->debug ) {
			...
			$granularity = '&nbsp;<b>' . __( 'Minutes', 'fw' ) . '</b>';
		} else {
			$description = __( 'Age limit of backups in days', 'fw' );
			$granularity = '&nbsp;<b>' . __( 'Days', 'fw' ) . '</b>';
		}
					...
					'options' => array(
							...
							'choices' => array(
								...
								'dropbox'      => array(
									...
									'disconnect' => array(
										'type'  => 'html',
										'label' => __( 'Dropbox', 'fw' ),
										'html'  => '<button data-action="backup-dropbox-disconnect" class="button button-primary">' . __( 'Disconnect', 'fw' ) . '</button>',
									),
								),
								'google_drive' => array(),
							),
						...
						"$cron_id-lifetime" => array(
							...
							'attr'  => array(
								'placeholder'     => __( 'No Limit', 'fw' ),
								'style'           => 'width: 90%;',
								'data-html-after' => '&nbsp;<b>' . __( 'Days', 'fw' ) . '</b>',
							),
						),
					),
				),
